...
import com.mgt.kongclient.model.admin.plugin.OAuth2Config;
...
public class RetrofitApiPluginServiceTest extends BaseTest {
  ...
  public void test02_GetPluginForApi() throws IOException {
    var response = kongClient.getApiPluginService().getPluginForApi(API_NAME, PLUGIN_ID);
    printJson(response);
  ...
  public void test04_UpdatePluginForApi() throws IOException {
    var request = new Plugin(null, PLUGIN_NAME, null);

    var response = kongClient.getApiPluginService().updatePluginForApi(API_NAME, PLUGIN_ID, request);
    printJson(response);
    Assert.assertEquals(request.name(), response.name());
  }
  ...
  public void test05_CreateOrUpdatePluginForApi() throws IOException {
    var request = new Plugin(PLUGIN_ID, PLUGIN_NAME, new Date().getTime());

    var response = kongClient.getApiPluginService().createOrUpdatePluginForApi(API_NAME, request);
    printJson(response);
    Assert.assertEquals(request.name(), response.name());
  }
